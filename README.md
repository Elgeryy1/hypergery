...
- **Console** opens a separate HyperGery Console window for VNC-backed running VMs.
- **External Console** keeps the existing `virt-viewer` / `remote-viewer` workflow as a fallback, including SPICE.
...
- Click inside the console window to capture input. Press **Right Ctrl** to release input.
- Closing or disconnecting the console window does not stop the VM; use ACPI Shutdown or Force Off to stop it.
- SPICE remains supported for external viewing. The HyperGery Console window shows a clear SPICE message and an external-viewer fallback instead of a blank screen.
- For the integrated console, choose display mode `vnc` when creating or editing a VM.
