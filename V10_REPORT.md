...
| Teleport Engine | **functional** (validado en real en una máquina) | `v1/teleport.py` | 11 + integración | dry_run, local_loopback y suspend_copy_start validados — los dos últimos en **KVM real**: local_loopback exporta+importa en este host; suspend_copy_start se probó host→host en una sola máquina (segundo agente + Hub local) importando a libvirt real con UUID/MAC regenerados y origen intacto. include_iso opcional. Falta solo el smoke con el PC de casa real (2 máquinas físicas). |
